...
have fixed computational cost $O(n^2)$, a \emph{$K$-d tree} (i.e.
...
\begin{figure}[htbp] \label{fig:kd-tree-subdiv}
\centering
  ...
  other. Image source:
https://upload.wikimedia.org/wikipedia/commons/b/b6/3dtree.png (GNU GPLv2) }
\end{figure}
...
higer number of nodes in one of the subtrees with respect to the other. However,
...
\end{tabular}
    \caption{A $K$-d tree, like the one shown on the top, subdivides the
      ...
      half of the space have a distance smaller than $d$ to $\vec{P}$ (center). If
